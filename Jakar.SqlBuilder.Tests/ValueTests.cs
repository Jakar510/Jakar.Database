...
{
    [Test] public void Inline_NumberLiteral() => Assert.That(SqlBuilder.PostgreSQL.Select("*").From("t").Where("age").GreaterThan(18).Build().Sql, Is.EqualTo("SELECT * FROM t WHERE age > 18;"));

    [Test] public void Inline_NegativeNumber() => Assert.That(SqlBuilder.PostgreSQL.Select("*").From("t").Where("balance").LessThan(-5).Build().Sql, Is.EqualTo("SELECT * FROM t WHERE balance < -5;"));
    ...
    [Test] public void Inline_StringEscapesSingleQuote() => Assert.That(SqlBuilder.PostgreSQL.Select("*").From("t").Where("name").EqualTo(SqlValue.Inline("O'Brien")).Build().Sql, Is.EqualTo("SELECT * FROM t WHERE name = 'O''Brien';"));

    [Test] public void Raw_EmittedVerbatim_NoParens_Counted() => Assert.That(SqlBuilder.PostgreSQL.Select("*").From("t").Where("ts").GreaterThan(SqlValue.Raw("NOW()")).Build().Sql, Is.EqualTo("SELECT * FROM t WHERE ts > NOW();"));
    ...
    {
        SqlResult r = SqlBuilder.PostgreSQL.Select("*").From("t").Where("a").EqualTo(SqlValue.Param(1)).And("b").EqualTo(SqlValue.Param(2)).And("c").EqualTo(SqlValue.Param(3)).Build();
    ...
    [Test] public void SqlValueOf_Long() => Assert.That(SqlBuilder.PostgreSQL.Select("*").From("t").Where("x").EqualTo(SqlValue.Of(7L)).Build().Sql, Is.EqualTo("SELECT * FROM t WHERE x = 7;"));

    [Test] public void SqlValueOf_Bool_Postgres() => Assert.That(SqlBuilder.PostgreSQL.Select("*").From("t").Where("x").EqualTo(SqlValue.Of(true)).Build().Sql, Is.EqualTo("SELECT * FROM t WHERE x = TRUE;"));

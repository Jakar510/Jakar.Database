...
public sealed class SelectTests
{
    ...
    [Test] public void Aggregate_WithAlias()   => Assert.That(SqlBuilder.PostgreSQL.Select("u.name").Count("o.id", "orders").From("users", "u").Build().Sql,
                                                              Is.EqualTo("SELECT u.name, COUNT(o.id) AS orders FROM users u;"));
    [Test] public void Aggregate_SumMinMaxAvg() => Assert.That(SqlBuilder.PostgreSQL.Select().Sum("a").Min("b").Max("c").Avg("d").From("t").Build().Sql,
                                                              Is.EqualTo("SELECT SUM(a), MIN(b), MAX(c), AVG(d) FROM t;"));

    // ---- joins ----
    [Test] public void Join_Inner() => Assert.That(SqlBuilder.PostgreSQL.Select("*").From("users", "u").InnerJoin("orders", "o").On("o.user_id").EqualToColumn("u.id").Build().Sql,
                                                   Is.EqualTo("SELECT * FROM users u INNER JOIN orders o ON o.user_id = u.id;"));
    [Test] public void Join_Left()  => Assert.That(SqlBuilder.PostgreSQL.Select("*").From("users", "u").LeftJoin("orders", "o").On("o.user_id").EqualToColumn("u.id").Build().Sql,
                                                   Is.EqualTo("SELECT * FROM users u LEFT JOIN orders o ON o.user_id = u.id;"));
    [Test] public void Join_Right() => Assert.That(SqlBuilder.PostgreSQL.Select("*").From("a").RightJoin("b").On("a.x").EqualToColumn("b.y").Build().Sql,
                                                   Is.EqualTo("SELECT * FROM a RIGHT JOIN b ON a.x = b.y;"));
    [Test] public void Join_Full()  => Assert.That(SqlBuilder.PostgreSQL.Select("*").From("a").FullJoin("b").On("a.x").EqualToColumn("b.y").Build().Sql,
                                                   Is.EqualTo("SELECT * FROM a FULL JOIN b ON a.x = b.y;"));
    [Test] public void Join_Cross() => Assert.That(SqlBuilder.PostgreSQL.Select("*").From("a").CrossJoin("b").Build().Sql,
                                                   Is.EqualTo("SELECT * FROM a CROSS JOIN b;"));
    ...
    [Test] public void Where_AllComparators()    => Assert.That(SqlBuilder.PostgreSQL.Select("*").From("t")
                                                                          .Where("a").EqualTo(1)
                                                                          .And("b").NotEqualTo(2)
                                                                          .And("c").GreaterOrEqual(3)
                                                                          .And("d").LessThan(4)
                                                                          .And("e").LessOrEqual(5).Build().Sql,
                                                              Is.EqualTo("SELECT * FROM t WHERE a = 1 AND b <> 2 AND c >= 3 AND d < 4 AND e <= 5;"));
    ...
    // ---- group / having ----
    [Test] public void GroupBy_Multiple() => Assert.That(SqlBuilder.PostgreSQL.Select("a", "b").Count("*", "n").From("t").GroupBy("a", "b").Build().Sql,
                                                        Is.EqualTo("SELECT a, b, COUNT(*) AS n FROM t GROUP BY a, b;"));
    [Test] public void Having_Aggregate() => Assert.That(SqlBuilder.PostgreSQL.Select("a").From("t").GroupBy("a").Having().Count("id").GreaterThan(5).Build().Sql,
                                                        Is.EqualTo("SELECT a FROM t GROUP BY a HAVING COUNT(id) > 5;"));
    ...
    // ---- set operations ----
    [Test] public void Union()    => Assert.That(SqlBuilder.PostgreSQL.Select("id").From("a").Where("active").EqualTo(true).Union().Select("id").From("b").Build().Sql,
                                                Is.EqualTo("SELECT id FROM a WHERE active = TRUE UNION SELECT id FROM b;"));
    [Test] public void UnionAll() => Assert.That(SqlBuilder.PostgreSQL.Select("id").From("a").UnionAll().Select("id").From("b").Build().Sql,
                                                Is.EqualTo("SELECT id FROM a UNION ALL SELECT id FROM b;"));

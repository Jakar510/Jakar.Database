...
public class UserStore( Database dbContext ) : IUserStore
{
    ...
    public virtual async Task<IList<UserRecord>> GetUsersForClaimAsync( Claim claim, CancellationToken token ) => await dbContext.GetUsersForClaimAsync(claim, token)
    ...
    public virtual async Task<IList<UserLoginInfo>> GetLoginsAsync( UserRecord user, CancellationToken token )
    {
        return await dbContext.GetLoginsAsync(user, token)
    ...
    public virtual async Task<int> CountCodesAsync( UserRecord user, CancellationToken token ) => ( await user.Codes(dbContext, token)

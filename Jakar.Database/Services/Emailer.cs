...
public class Emailer( EmailTokenProvider tokenProvider, IConfiguration configuration, ILogger<Emailer> logger, Database dataBase, IOptions<EmailerOptions> options ) : IEmailer
{
    ...
    public async Task VerifyEmail( UserRecord user, ClaimType types, CancellationToken token )
    {
        ...
        EmailBuilder builder = EmailBuilder.From(_options.GetSender()).To(MailboxAddress.Parse(Validate.ThrowIfNull(user.Email))).WithSubject(subject).WithBody(content);
    ...
    public async Task VerifyHTMLEmail( UserRecord user, ClaimType types, CancellationToken token )
    {
        ...
        EmailBuilder builder = EmailBuilder.From(_options.GetSender()).To(MailboxAddress.Parse(Validate.ThrowIfNull(user.Email))).WithSubject(subject).WithHTML(content);

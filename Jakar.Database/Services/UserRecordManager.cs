                          ...
                          ILogger<UserManager>                        logger ) : UserManager<UserRecord>(store, optionsAccessor, passwordHasher, userValidators, passwordValidators, keyNormalizer, errors, services, logger)
{
    ...
    public async          ValueTask<IList<Claim>> GetClaimsAsync( DbConnectionContext context, UserRecord user, CancellationToken token ) => await user.GetUserClaims(context, _database, Claims.DEFAULTS, token);
}

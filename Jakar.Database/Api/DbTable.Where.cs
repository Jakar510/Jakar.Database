...
public partial class DbTable<TSelf>
{
    ...
    public virtual IAsyncEnumerable<TSelf> Where<TValue>( NpgsqlConnection connection, NpgsqlTransaction? transaction, string columnName, TValue? value, [EnumeratorCancellation] CancellationToken token = default )
    {
        ...
        SqlCommand sql = new($"SELECT * FROM {TSelf.TableName} WHERE @{nameof(columnName)} = @{nameof(value)};", parameters);
    ...
    public IAsyncEnumerable<RecordID<TSelf>> WhereID( string         sql,        PostgresParameters parameters, [EnumeratorCancellation] CancellationToken token = default ) => this.TryCall(WhereID, new SqlCommand(sql, parameters), token);
    ...
    public virtual IAsyncEnumerable<RecordID<TSelf>> WhereID( NpgsqlConnection connection, NpgsqlTransaction? transaction, string sql, PostgresParameters parameters, [EnumeratorCancellation] CancellationToken token = default ) => WhereID(connection, transaction, new SqlCommand(sql, parameters), token);
    ...
    public virtual IAsyncEnumerable<RecordID<TSelf>> WhereID<TValue>( NpgsqlConnection connection, NpgsqlTransaction? transaction, string columnName, TValue? value, [EnumeratorCancellation] CancellationToken token = default )
    {
        ...
        SqlCommand sql = new($"SELECT {nameof(IUniqueID.ID).SqlName()} FROM {TSelf.TableName} WHERE @{nameof(columnName)} = @{nameof(value)};", parameters);
